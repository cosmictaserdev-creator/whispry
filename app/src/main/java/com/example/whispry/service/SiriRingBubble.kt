...
fun SiriRingBubble(
    ...
    val time by rememberInfiniteTransition(label = "VisualizerTime").animateFloat(
        initialValue = 0f,
    ...
    Canvas(modifier = modifier.fillMaxSize()) {
        ...
        drawMorphingRing(center, baseRadius, time, currentAmplitude, colorA, 0)
        drawMorphingRing(center, baseRadius, time * 1.2f, currentAmplitude * 0.8f, colorB, 1)
        drawMorphingRing(center, baseRadius, time * 0.8f, currentAmplitude * 1.1f, Color.White, 2)
    }
}
...
private fun DrawScope.drawMorphingRing(
    ...
    val path = Path() // Still need a path, but let's minimize other allocations
